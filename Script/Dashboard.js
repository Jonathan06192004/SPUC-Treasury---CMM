...
// Fetch CMM data from database
async function fetchCMMData() {
  try {
    ...
    if (missionError || !mission) {
      console.error('Error fetching CMM mission:', missionError);
      initializeWithFallbackData();
      return;
    }

    CMM_MISSION_ID = mission.id;

    const { data: districts, error: districtError } = await sb
      .from('districts')
      .select('id')
      .eq('mission_id', CMM_MISSION_ID);
    ...
    console.error('Error fetching CMM data:', error);
    initializeWithFallbackData();
  }
}
